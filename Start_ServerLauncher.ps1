...
function New-CyberWallpaper {
    ...
    try {
        ...
        $brush = New-Object System.Drawing.Drawing2D.LinearGradientBrush($rect,[System.Drawing.Color]::FromArgb(7,13,25),[System.Drawing.Color]::FromArgb(12,28,44),90)
        $g.FillRectangle($brush,$rect)
        $brush.Dispose()

        # Distant city / server-rack skyline
        $rand = New-Object System.Random 42
        for($x=0;$x -lt $w;$x+=48){
            $bh = $rand.Next(220,650)
            $y = $h-$bh
            $body = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(225,8,16,28))
            $g.FillRectangle($body,$x,$y,39,$bh)
            $body.Dispose()
            for($wy=$y+24;$wy -lt $h-35;$wy+=34){
                ...
                $light = New-Object System.Drawing.SolidBrush($c)
                $g.FillRectangle($light,$x+8,$wy,22,5)
                $light.Dispose()
            }
        }

        # Horizon haze
        $haze = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(80,0,180,220))
        $g.FillRectangle($haze,0,590,$w,5)
        $haze.Dispose()

        # Wet floor / reflections
        for($i=0;$i -lt 25;$i++){
            $yy = 610 + ($i*11)
            $alpha = [Math]::Max(8,70-($i*2))
            $pen1 = New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb($alpha,0,205,255),2)
            $pen2 = New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb($alpha,255,40,110),2)
            $g.DrawLine($pen1,100,$yy,720+$i*8,$yy)
            $g.DrawLine($pen2,880-$i*7,$yy,1510,$yy)
            $pen1.Dispose(); $pen2.Dispose()
        }

        # Subtle stylized operator silhouette on the left, deliberately abstract.
        $sil = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(175,3,8,16))
        $g.FillEllipse($sil,105,135,300,300)
        $g.FillPie($sil,45,330,470,600,190,160)
        $sil.Dispose()
        $edge = New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb(180,0,210,255),5)
        $g.DrawArc($edge,105,135,300,300,120,180)
        $edge.Dispose()
        $edge2 = New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb(160,255,35,110),4)
        $g.DrawArc($edge2,70,280,430,500,205,95)
        $edge2.Dispose()

        # Keep center/right readable for the launcher UI.
        $shade = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(70,0,0,0))
        $g.FillRectangle($shade,500,0,1100,900)
        $shade.Dispose()
    } finally {
        $g.Dispose()
    }
...
function New-NeonIcon {
    ...
    try {
        $g.SmoothingMode=[System.Drawing.Drawing2D.SmoothingMode]::AntiAlias
        $cyan=[System.Drawing.Color]::FromArgb(0,215,255)
        $pink=[System.Drawing.Color]::FromArgb(255,55,145)
        $pen=New-Object System.Drawing.Pen($cyan,6)
        $pen2=New-Object System.Drawing.Pen($pink,4)
        switch($Kind){
            ...
            'VM' { $pts=@((New-Object Drawing.Point 48,13),(New-Object Drawing.Point 76,30),(New-Object Drawing.Point 76,64),(New-Object Drawing.Point 48,82),(New-Object Drawing.Point 20,64),(New-Object Drawing.Point 20,30)); $g.DrawPolygon($pen,$pts); $g.DrawLine($pen,20,30,48,48); $g.DrawLine($pen,76,30,48,48); $g.DrawLine($pen,48,48,48,82) }
...
if (-not (Test-Path -LiteralPath $settingsFile)) {
    ...
    $json=$defaults | ConvertTo-Json
    [System.IO.File]::WriteAllText($settingsFile,$json,(New-Object System.Text.UTF8Encoding($true)))
}
