# HypeTek Bookmarker - runtime DPI-awareness probe
# Run while the packaged Store-test app is open. Does not require elevation.

$ErrorActionPreference = 'Stop'

$process = Get-Process -Name 'HypeTek-Bookmarker' -ErrorAction SilentlyContinue |
    Where-Object { $_.MainWindowHandle -ne 0 } |
    Select-Object -First 1

if (-not $process) {
    Write-Host 'HypeTek Bookmarker is not running with a visible main window.' -ForegroundColor Yellow
...
Write-Host 'HypeTek Bookmarker - runtime DPI-awareness probe'
